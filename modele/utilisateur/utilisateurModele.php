...
class Utilisateur
{
    ...
    public function ajouterClient($idclient, $code_postal, $date_naissance)
    {
        try
        {
            ...
            $requeteT = "INSERT INTO Client (idclient, code_postal, date_naissance) VALUES ($idclient, $code_postal, $date_naissance);";

            $insert = $this->bdd->prepare($requete);

            $insert->bindParam(':idclient', $idclient, PDO::PARAM_INT);
            $insert->bindParam(':code_postal', $code_postal, PDO::PARAM_INT);
            $insert->bindParam(':date_naissance', $date_naissance, PDO::PARAM_STR);

            echo $date_naissance;
            echo ('requetT : ' . var_dump($requeteT));
     //       echo ('Insert : ' . var_dump($insert));
